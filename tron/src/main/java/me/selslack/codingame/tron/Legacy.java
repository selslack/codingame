...
class GameRules {
    ...
    static public int getCellOwner(GameState state, int x, int y) {
        return state.getCellValue(x, y);
    }

    static public int getCellOwner(GameState state, int x, int y, Direction direction) {
        return state.getCellValue(x + direction.getX(), y + direction.getY());
    }
    ...
    static public int getNormalizedCellCountAvailable(GameState state, int playerId) {
        int[]            players   = new int[state.getPlayerCount()];
        HashSet<Cell>    checked   = new HashSet<>(256);
        ...
        // Initial position
        stack.add(new Cell(state.getPosition(playerId)));
        checked.add(stack.getLast());

        while (!stack.isEmpty()) {
            Cell cell = stack.remove();

            if (isCellAvailable(state, cell.getX(), cell.getY())) {
                result++;
            }

            for (Direction direction : Direction.values()) {
                if (isCellAvailable(state, cell.getX(), cell.getY(), direction)) {
                    Cell visiting = new Cell(cell, direction);

                    if (!checked.contains(visiting)) {
                        stack.add(visiting);
                        checked.add(visiting);
                    }
                }
                else {
                    int owner = getCellOwner(state, cell.getX(), cell.getY(), direction);
        ...
        return result / (int) Arrays.stream(players).filter(v -> v == 1).count();
    }
...
class PossibleMovementsEvaluator implements Evaluator {
    @Override
    public int evaluate(GameState state, int playerId) {
        return GameRules.getNormalizedCellCountAvailable(state, playerId) * 10;
    }
}
